...
-- does not combine multiple spaces into a single token
spaces :: Parser Chars
spaces = list space
...
option fallback pa = pa ||| valueParser fallback
...
oneof chrs = let
  parsers :: List (Parser Char)
  parsers = is <$> chrs
  in foldRight (|||) failed parsers
